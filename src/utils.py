...
# this is the function for a tupled dice 
...
def dice_score_3d(predictions: torch.Tensor, targets: torch.Tensor, epsilon: float = 1e-6):
    """
    Calculates Dice score for 3D volumes (5D tensors).
    """
    # 1. Convert logits to a predicted mask
    pred_mask = torch.softmax(predictions, dim=1).argmax(dim=1)
    num_classes = predictions.shape[1]
    
    # 2. One-hot encode both prediction and target for 3D
    # (B, D, H, W) -> (B, D, H, W, C) -> (B, C, D, H, W)
    pred_mask_one_hot = F.one_hot(pred_mask, num_classes=num_classes).permute(0, 4, 1, 2, 3).float()
    # (B, 1, D, H, W) -> (B, D, H, W) -> (B, D, H, W, C) -> (B, C, D, H, W)
    targets_one_hot = F.one_hot(targets.squeeze(1), num_classes=num_classes).permute(0, 4, 1, 2, 3).float()

    # 3. Calculate intersection and union over all dims except class
    dims = (0, 2, 3, 4) # Sum over Batch, Depth, Height, and Width
    intersection = (pred_mask_one_hot * targets_one_hot).sum(dim=dims)
    union = pred_mask_one_hot.sum(dim=dims) + targets_one_hot.sum(dim=dims)
